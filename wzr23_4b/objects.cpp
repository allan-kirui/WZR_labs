...
MovableObject::MovableObject(Terrain *t)             // konstruktor                   
{
	...
	pieniadze = 2000;    // np. dolarów
	ilosc_paliwa = 15.0;   // np. kilogramów paliwa 
	...
	F_max = 5000;
	alfa_max = PI*25.0 / 180;
	masa_wlasna = 1000.0;     // masa w³asna obiektu [kg] (bez paliwa)
	...
	dl_tyl = 0.4;             // odleg³oœæ od tylniej osi do tylniego zderzaka

	iID_kolid = -1;           // na razie brak kolizji 

	//wPol.y = przeswit+wysokosc/2 + 10;
	wPol = Wektor3(220, przeswit + wysokosc / 2 + 30, -150);
	...
	kwaternion qObr = AsixToQuat(Wektor3(0, 1, 0), -70 * PI / 180.0);
...
{
	bool czy_z_pliku = 1;
    szczeg = 0.6;                 // stopieñ szczegó³owoœci wyœwietlania przedmiotów i powierzchni terrainu (1 - pe³na, 0 - minimalna)
	liczba_wyswietlen = 0;
	ts = new TabSektorow();
	...
	if (czy_z_pliku)
	{
		char filename[] = "paralaksa3_2_3.map";
...
void Terrain::Rysuj()
{
	...
	GLfloat GreenSurface[] = { 0.35f, 0.72f, 0.1f, 1.0f };
