...
void InteractionInitialisation()
{
	...
	multi_reciv = new multicast_net("224.12.13.213", 10001);     
	multi_send = new multicast_net("224.12.13.213", 10001);      
...
void VirtualWorldCycle()
{
	...
	{                              
		...
		sprintf(text, "WZR 2022/23 ver. b, avg.freq = %0.2f[frame/s]  avg.dist = %0.3f[m]  avg.ang.dist = %0.3f[deg]",
	...
	for (map<int, MovableObject*>::iterator it = movable_objects.begin(); it != movable_objects.end(); ++it)
	{
		...
		if (veh)
		{
			...
			//veh->state.vPos = ...
			veh->state.vPos = veh->state.vV * fDt + veh->state.vPos + veh->state.vA * fDt * fDt / 2;
			float distFromGround = terrain.DistFromGround(veh->state.vPos.x, veh->state.vPos.z);
			if (distFromGround >= veh->state.vPos.y + veh->height)
			{
				veh->state.vPos.y = veh->height + distFromGround;
				//veh->state.vV.y = 0.0;
				//veh->state.vA.y = 0.0;
			}
			//veh->state.vV = veh->state.
			veh->state.vV += veh->state.vA * fDt;
			//veh->state.qOrient = ....
			...
			//veh->state.vV_ang = ....
			veh->state.vV_ang = veh->state.vA_ang * fDt;
		}
	}
	//Release the Critical section
	LeaveCriticalSection(&m_cs);
	...
	int       nCmdShow)
{
	...
	window_handle = CreateWindow(class_name, "WZR-lab 2022/23 ver. b - state prediction", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
