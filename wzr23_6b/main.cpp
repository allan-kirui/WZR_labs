...
enum frame_types {
	OBJECT_STATE, ITEM_TAKING, ITEM_RENEWAL, COLLISION, TRANSFER, NEGOTIATION, AUCTION
};

enum transfer_types { MONEY, FUEL };
enum auction_type { STATE, STOP, BID };

int auctionHostID = 0;
float auctionTimeLeft = 0;
float auctionCurrentBid = 0;
float auctionNextBid = 0;
...
int collabID = 0;
transfer_types collabTransferType = MONEY;
...
struct Frame
{
	...
	float auctionCurrentBid;
	float auctionNextBid;
	float auctionTimeLeft;
	
	long existing_time;        // czas jaki uplyn¹³ od uruchomienia programu
};

void NegotiationSending(int ID_receiver, int transfer_type)
{
	Frame frame;
...
DWORD WINAPI ReceiveThreadFunction(void *ptr)
{
	...
	while (1)
	{
		...
		case AUCTION:
		{
			...
			if (frame.auctionType == STATE) {
				// auction //
				if (auctionHostID == 0) { auctionHostID = frame.iID; }
				if (auctionHostID != frame.iID) { break; }

				auctionCurrentBidID = frame.iID_receiver;
				auctionCurrentBid = frame.auctionCurrentBid;
				auctionNextBid = frame.auctionNextBid;
				auctionTimeLeft = frame.auctionTimeLeft;

				if (auctionCurrentBidID == my_vehicle->iID) {
					// show ure winning //
				}
				else {
					// show ure loosing //
			...
			else if (frame.auctionType == STOP) {
				if (auctionHostID == 0) { auctionHostID = frame.iID; }
				if (auctionHostID != frame.iID) { break; }

				if (frame.iID_receiver == my_vehicle->iID) {
					// transfer cash //
					TransferSending(auctionHostID, MONEY, frame.auctionCurrentBid);
				}
				else {
					// nic //
				}
				...
				auctionHostID = 0;
				auctionCurrentBidID = 0;
				auctionCurrentBid = 0;
				auctionNextBid = 0;
				auctionTimeLeft = 0;
			}
			else if (frame.auctionType == BID && auctionHostID == my_vehicle->iID) {
				// for host //
				if (frame.auctionNextBid > auctionCurrentBid) {
					// new higher bid //
					auctionCurrentBid = frame.auctionNextBid;
					auctionCurrentBidID = frame.iID;
					auctionNextBid = auctionCurrentBid * (1.1);
					// show new bid in interface //
					AuctionSendState();
				}
			}
			break;
		...
		case NEGOTIATION:
		{
			if (frame.iID_receiver == my_vehicle->iID)  // ID pojazdu, ktory otrzymal przelew zgadza siê z moim ID 
			{
				if (frame.iID == myRequestedID && frame.transfer_type != myRequestedTransferType)
				{
					collabID = frame.iID;
					collabTransferType = (transfer_types)frame.transfer_type;
				}
				else
				{
					int pID = frame.iID;
					if (frame.transfer_type == MONEY)
					{
						...
						int msgBoxID = MessageBox(NULL, (LPCSTR)"Do you accept money from me?", (LPCSTR)"Negotiation offer", MB_ICONWARNING | MB_YESNO);
						...
						if (accepted)
						{
							NegotiationSending(frame.iID, FUEL);
							collabID = frame.iID;
							collabTransferType = (transfer_types)frame.transfer_type;
							myRequestedID = frame.iID;
						}
						else
						{
							// pass
					...
					else if (frame.transfer_type == FUEL)
					{
						...
						int msgBoxID = MessageBox(NULL, (LPCSTR)"Do you accept fuel from me?", (LPCSTR)"Negotiation offer", MB_ICONWARNING | MB_YESNO);
						...
						if (accepted)
						{
							NegotiationSending(frame.iID, MONEY);
							collabID = frame.iID;
							collabTransferType = (transfer_types)frame.transfer_type;
							myRequestedID = frame.iID;
						}
...
void VirtualWorldCycle()
{
	...
	if (my_vehicle->number_of_taking_item > -1)
	{
		...
		if (collabID != 0)
		{
			float amountToTransfer = my_vehicle->taking_value / 2;

			int itemType = terrain.p[my_vehicle->number_of_taking_item].type;
			transfer_types transferType = MONEY;
			if (itemType == ITEM_BARREL) { transferType = FUEL; }

			if (transferType == collabTransferType)
			{
				TransferSending(collabID, collabTransferType, amountToTransfer);
			}
		}
...
float TransferSending(int ID_receiver, int transfer_type, float transfer_value)
{
	...
	else if (transfer_type == NEGOTIATION) {
		//SetWindowText();
	}
	...
	case WM_KEYDOWN:
	{
		...
		case 'N':
		{
			for (map<int, MovableObject*>::iterator it = network_vehicles.begin(); it != network_vehicles.end(); ++it)
			{
				...
				{
					MovableObject* ob = it->second;
					NegotiationSending(ob->iID, MONEY);
				}
			}
		}
		...
		case 'P':
		{
			if (auctionHostID != 0) { break; }
			auctionHostID = my_vehicle->iID;
		...
		default:
		{
			...
			{
				prepRequestedID = 10 * prepRequestedID + LOWORD(wParam) - '0';
			}
		}
